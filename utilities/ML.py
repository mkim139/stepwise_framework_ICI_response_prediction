...
def expression_StandardScaler2(train_exp_df,test_exp_df):
	'''
	Input : expression dataframe
	'''
	ss = StandardScaler()

	col1 = train_exp_df.columns[0]
	col2 = test_exp_df.columns[0]

	
	tmp = ss.fit_transform(train_exp_df.T.values[1:])
	new_tmp = defaultdict(list)
	new_tmp[col1] = train_exp_df[col1].tolist()
	for s_idx, sample in enumerate(train_exp_df.columns[1:]):
		new_tmp[sample] = tmp[s_idx]
	output = pd.DataFrame(data=new_tmp, columns=train_exp_df.columns)

	
	tmp = ss.transform(test_exp_df.T.values[1:])
	new_tmp = defaultdict(list)
	new_tmp[col2] = test_exp_df[col2].tolist()
	for s_idx, sample in enumerate(test_exp_df.columns[1:]):
		new_tmp[sample] = tmp[s_idx]
	output2 = pd.DataFrame(data=new_tmp, columns=test_exp_df.columns)
	return output, output2

from sklearn.preprocessing import MinMaxScaler

def expression_StandardScaler3(train_exp_df,test_exp_df):
	'''
	Input : expression dataframe
	'''
	ss = StandardScaler()
	...
	trainshape = train_exp_df.shape[0]
	testshape = test_exp_df.shape[0]
	train_exp_df = pd.concat([train_exp_df, test_exp_df], axis=0)
	...
	for s_idx, sample in enumerate(train_exp_df.columns[1:]):
		new_tmp[sample] = tmp[s_idx]
	output = pd.DataFrame(data=new_tmp, columns=train_exp_df.columns)
	output1,output2 = output.iloc[:,:trainshape+1],output.iloc[:,[0]+list(range(trainshape+1,trainshape+testshape+1))]
...
def ML_hyperparameters(ML,X_train=None,penalty='l2'):
	'''
	Input
	...
	if ML == 'LogisticRegression':
		...
		if penalty == 'l2':
			param_grid = {'penalty':['l2'], 'max_iter':[int(1e10)], 'solver':['lbfgs'], 'C':np.arange(0.1, 1, 0.1), 'class_weight':['balanced'] }
		if penalty == 'none':
